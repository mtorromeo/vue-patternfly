<template>
  <component
    ...
    <span v-if="onClose || $slots.actions" :class="styles.labelActions">
      <slot name="actions">
        <pf-button variant="plain" no-padding @click="onClose" :aria-label="closeBtnAriaLabel" :disabled="isClickableDisabled ? true : undefined">
          <template #icon>
            <xmark-icon />
          </template>
        </pf-button>
...
import { ref, type Ref, type ButtonHTMLAttributes, type VNode, computed } from 'vue';
